...
3. nextjs 를 이용하여 프론트엔드에서 cors 해결하기: nextjs 를 이용해서 custom request handler 를 만들게 되면 cors 문제를 프론트에서도 해결 하는 것이 가능하다.
...
위처럼 `Access-Control-Allow-Origin` 에 대한 필드를 \* 로 주어 어떠한 요청도 다 처리 가능한 형태로 수정하는 것이 가능합니다.

마지막으로 `node server.ts` 를 실행해 서버에서 넘어오는 origin 을 프론트에서 원하는 형태로 조정하는 것이 가능해집니다.
