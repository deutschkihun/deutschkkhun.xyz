...
date: '2022-12-12'
lastmod: '2022-12-14'
tags: ['algorithm', 'sorting', 'data-structure']
draft: false
...
### 1.3 array 와 linkedlist 는 어떤 데이터에 사용하면 좋을까?

- 필자는 array 데이터가 정해진 크기를 가지고 있고, 빠른 접근이 필요한 경우 쓰면 좋을 것이라 생각합니다.

- 필자는 linkedlist 는 데이터의 크기가 가변적이고, 삽입/삭제가 빈번하게 일어나는 경우에 사용하면 좋을것이라 생각한다. 예를 들어, 게시판의 글 목록에서 글을 추가하거나 삭제할 때 연결리스트를 사용할 수 있습니다. 삽입/삭제 시에는 단순히 노드의 포인터만 변경하면 되기 때문에, 배열보다 효율적입니다.
