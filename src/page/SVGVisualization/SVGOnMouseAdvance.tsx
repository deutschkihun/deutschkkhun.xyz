...
interface SVGOnMouseAdvanceProps {
  width: number;
  height: number;
}

export const SVGOnMouseAdvance = ({
  width,
  height,
}: SVGOnMouseAdvanceProps): JSX.Element => {
  const init = { x: width / 2, y: height / 2 };
  const MouseMoveRefAdvance = useRef<SVGSVGElement>(null);
  const [mousePosition, setMousePosition] = useState(init);
  const handleMouseMove = useCallback(
    (e) => {
      if (MouseMoveRefAdvance.current) {
        const leftOffSet =
          MouseMoveRefAdvance.current.getBoundingClientRect().left;
        const TopOffSet =
          MouseMoveRefAdvance.current.getBoundingClientRect().top;
  ...
  return (
    ...
    >
      <defs>
        ...
        <mask id="mask-glass">
          <circle
            cx={mousePosition.x / 1.5}
            cy={mousePosition.y / 1.5}
            r="70"
            fill="#fff"
          />
        </mask>
